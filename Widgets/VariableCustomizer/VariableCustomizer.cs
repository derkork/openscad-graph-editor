...
namespace OpenScadGraphEditor.Widgets.VariableCustomizer
{
    ...
    public class VariableCustomizer : MarginContainer
    {
        ...
        public void Setup(IEditorContext context, List<VariableDescription> variables)
        {
            ...
            // add a section for variables that don't appear in the customizer
            var otherVariables = variables
                .Where(it => !it.ShowInCustomizer)
                .ToList();
            variableGroups.AddRange(otherVariables.GroupBy(it => "[Not in customizer]"));
            ...
            var unusedVariables = _variables.Keys.Except(variables
        ...
        private Control CreateOrGetSection(string name)
        {
            ...
            toggle.ButtonToggled += (pressed) => { insertPoint.Visible = pressed; };
        ...
        private Control CreateOrGetVariable(VariableDescription variable)
        {
            ...
            var descriptionLabel = result.WithName<Label>("VariableDescription");
            descriptionLabel.Text = variable.Description;
            // only show the description if it's not empty
            descriptionLabel.Visible = variable.Description.Trim().Length > 0;
            ...
            if (literal != null)
            {
                ...
                var newWidget = copy.BuildWidget(false, true, false, variableControl as IScadLiteralWidget,
                    variable.CustomizerDescription);
